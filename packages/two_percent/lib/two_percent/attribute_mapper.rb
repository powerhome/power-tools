...
module TwoPercent
  ...
  class AttributeMapper
    ...
    def build_scim_representation(record, resource_type: "User")
      ...
      scim_hash["id"] = scim_hash.delete("scimId") if scim_hash["scimId"]
    ...
    def extract_model_attributes(scim_hash)
      ...
      @mapping.each do |scim_attr, model_attr|
        ...
        next unless value.present? && model_attr.is_a?(Symbol)
      ...
      attributes[@scim_data_column] = unmapped_data if @scim_data_column
  ...
  private
    ...
    def build_meta(record, resource_type)
      ...
      meta["created"] = record.created_at.iso8601 if record.respond_to?(:created_at) && record.created_at

      meta["lastModified"] = record.updated_at.iso8601 if record.respond_to?(:updated_at) && record.updated_at
