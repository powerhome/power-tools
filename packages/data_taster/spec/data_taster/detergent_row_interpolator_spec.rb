...
RSpec.describe DataTaster::DetergentRowInterpolator do
  describe ".replace_values" do
    ...
    it "replaces longer identifier names before shorter ones" do
      ...
      expect(described_class.replace_values(expression, row, client)).to eq("CONCAT(1, '-', 99)")
    end
    ...
    it "leaves non-identifier tokens unchanged" do
      ...
      expect(described_class.replace_values(expression, row, client)).to eq("CONCAT('users_', 5, '@nitrophrg.com')")
    end
  end
end
