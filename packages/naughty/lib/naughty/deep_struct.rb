...
module Naughty
  ...
  # the keys of the hash like methods, ie `Naughty::DeepStruct.new(foo: { bar: 'baz' }).foo.bar`
  #
  # It also uses the Null Object pattern so that any missing keys/methods called
  # will return a NullObject instead of errors.
  #
  # @example
  #   api_data = Naughty::DeepStruct.new(id: 10, name: "Frank", address: { street: "Orchid Avenue" })
